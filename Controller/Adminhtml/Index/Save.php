        ...
        try {
            ...
            if (!empty($data)) {
                ...
                if (!isset($integrationData[Main::DATA_ID])) {
                    $integration = $this->_service->create($integrationData);
                } else {
                    $integration = $this->_service->update($integrationData);
                }
                if (!$this->getRequest()->isXmlHttpRequest()) {
                    $this->messageManager->addSuccess(
                            __(
                                    'The integration \'%1\' has been saved.', $this->escaper->escapeHtml($integration->getName())
                            )
                    );
                }
                if ($this->getRequest()->isXmlHttpRequest()) {
